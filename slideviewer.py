...
class SlideViewer(Gtk.EventBox):
    ...
    def can_undo_redo(self):
        if self.__deck.getActiveSubmission() == -1 or self.__is_instr:
            if self.__is_instr:
                return ((len(self.__canvas.instr_ink) > 0),
                        (len(self.__canvas.redo_stack) > 0))
            else:
                return ((len(self.__canvas.self_ink) > 0),
                        (len(self.__canvas.redo_stack) > 0))
        else:
            return (False, False)
...
class SlideViewerCanvas(Gtk.DrawingArea):
    ...
    def _do_draw_cb(self, widget, context):
        ...
        self.__logger.debug("Exposing slide took " +
                            str(time.time() - timerstart) + " seconds")
...
class ThumbViewer(Gtk.DrawingArea):
    ...
    def __init__(self, deck, renderer, n):
        ...
        # Load thumbnail from the PNG file, if it exists; otherwise draw from
        # scratch
        timerstart = time.time()
        ...
        self.__logger.debug("Thumbnail loading/drawing took " +
                            str(time.time() - timerstart) + " seconds")
    ...
    def _do_draw_cb(self, widget, ctx):
        ...
        if self.__surface:
            ...
            ctx.fill()
        self.__logger.debug("Exposing slide thumbnail took " +
                            str(time.time() - timerstart) + " seconds")
